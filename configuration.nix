...

{
  ...
  users.users.${username} = {
    isNormalUser = true;
    description = "Joe";
    extraGroups = [ "networkmanager" "wheel" "video" "audio" "input" "render" "lp" ];
    shell = pkgs.zsh;
  };
  ...
  fileSystems."/mnt/truenas/home" = {
    ...
    fsType = "nfs";
    options = [
      "nfsvers=4.2" "x-systemd.automount" "noauto"
      "x-systemd.idle-timeout=600" "x-systemd.device-timeout=5s"
      "x-systemd.mount-timeout=5s" "soft" "timeo=14" "retrans=2" "_netdev"
    ];
  };
  ...
  # PROGRAMS
  # ============================================
  programs = { zsh.enable = true; dconf.enable = true; };

  # ============================================
  # PACKAGES
  # ============================================
  environment.systemPackages = with pkgs; [
    git wget curl vim htop btop tree file unzip zip ripgrep fd jq
    pciutils usbutils nfs-utils stow
  ];
